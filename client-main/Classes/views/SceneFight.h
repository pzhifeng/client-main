...
public:
	...
	void removeBallFromUI(int removeStart,int removeEnd,bool isExplosion);//´Ó½çÃæÉ¾³ýÇò
	void removeBallFromArray(int removeStart,int removeEnd);//´ÓÇòÊý×éÉ¾³ý
	bool isMoving();//ÅÐ¶ÏÊÇ·ñÓÐÇòÕýÔÚÒÆ¶¯
	void removeBallFromGoFinish();//µ½´ïÖÕµãÒÆ³ýËùÓÐÇò£¬ÓÎÏ·½áÊø
	void NPCDebuff();


	void initWayPoint();//³õÊ¼»¯µØÍ¼
	void insertBall(int hitIndex,char insertSide,int color,bool isPlayer);//²åÈëÇòµ½¹ìµÀ
